"""Diagnose a residual-decoder DVFM under four latent inputs.
...
The original encoder, prior, posterior, Weibull likelihood, and C-ELBO are unchanged. One trained
...
from .residual_core import ResidualDVFM
from . import semi_synthetic_frailty_prediction as _semi_synthetic_module
...
def run(config_path: Path) -> None:
    # train_and_measure_latent resolves DVFM from its defining module's
    # globals. Replace only that constructor; all training logic remains
    # unchanged.
    _semi_synthetic_module.DVFM = ResidualDVFM
    ...
    print("[4/7] Training DVFM with residual x-only decoder")
    set_seed(seed + 1000)
    (
        latent_model,
        validation_latent_predictions,
    ...
    latent_results["decoder"] = latent_model.decoder_diagnostics()
    ...
    print()
    print("Residual-decoder DVFM diagnostic")
    print("----------------------------")
    print(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate one residual DVFM decoder under posterior, true, "
