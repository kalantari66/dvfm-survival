...
3. samples Gaussian- or Clayton-copula survival quantiles with pycop;
4. creates complete E/C and observed T/delta;
5. writes the semi-synthetic CSV;
6. splits and preprocesses the generated data;
7. trains DVFM from dvfm.reference_core;
8. compares the encoder posterior mean with a known normalized Gaussian
   shared direction;
9. writes metrics and per-subject latent predictions.

Interpretation
--------------
For either copula, the uniform coordinates are transformed with Phi^{-1}
...
def train_and_measure_latent(
    ...
    config: dict[str, Any],
) -> tuple[DVFM, pd.DataFrame, dict[str, Any]]:
    """
    ...
    - optional checkpoint selection by validation absolute Pearson correlation;
    - early stopping;
    ...
    selection_metric = str(
        recovery_cfg.get("selection_metric", "validation_abs_pearson")
    )
    ...
    for epoch in range(epochs):
        ...
        if selection_metric == "validation_abs_pearson":
            score = valid_abs_pearson
        elif selection_metric == "validation_loss":
            score = -val_loss
        else:
            raise ValueError(
                "latent_recovery.selection_metric must be "
                "'validation_abs_pearson' or 'validation_loss'."
            )
    ...
    if recovery_cfg.get("report_subgroups", True):
        for subgroup_name, mask in {
            "uncensored": latent_frame["event"].to_numpy() == 1,
            "censored": latent_frame["event"].to_numpy() == 0,
        }.items():
            if mask.sum() >= 3:
    ...
    return model, latent_frame, metrics
...
def run(config_path: Path) -> None:
    ...
    model, latent_predictions, latent_results = train_and_measure_latent(
        train,
        valid,
        test,
        config,
