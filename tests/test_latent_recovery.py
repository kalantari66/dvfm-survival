...
def test_semisynthetic_runner_exports_each_tau_and_repeat(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(semi, "fit_support_semisynthetic_dgp", lambda *a, **kw: dgp)
    ...
    assert results.Scenario.nunique() == 2
    exports = list((tmp_path / "latent_recovery").glob("*/latent_recovery_metadata.json"))
    assert len(exports) == 4
    assert len(list((tmp_path / "latent_recovery").glob("*/latent_recovery_test.csv"))) == 2
