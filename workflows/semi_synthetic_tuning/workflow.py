"""One 12-hour GPU GWF target per semi-synthetic tuning dataset.
...
def _options(resources: dict, *, gpu: bool = True) -> dict[str, str]:
    options = {
...
def dataset_target(name: str, result_dir: Path, resources: dict) -> AnonymousTarget:
    ...
    echo "[GWF] $(date) starting semi-synthetic tuning: {name}"
    ...
    echo "[GWF] $(date) completed semi-synthetic tuning: {name}"
    """
    inputs = [
...
def aggregate_target(result_root: Path, names: list[str], resources: dict) -> AnonymousTarget:
    ...
    return AnonymousTarget(
        inputs=[str(path) for path in inputs], outputs=[str(path) for path in outputs],
        options=_options(resources, gpu=False), spec=spec,
    )
