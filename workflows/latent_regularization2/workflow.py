"""One-job GWF workflow for the paired DVFM latent-regularization v2 run.
...
CONFIG_PATH = PROJECT_ROOT / "configs" / "synthetic_latent_regularization2.yaml"
...
def run_ablation(config_path: Path, result_dir: Path, resources: dict) -> AnonymousTarget:
    ...
    echo "[GWF] $(date) starting DVFM latent-regularization v2"
    ...
    {checks}
    touch "{outputs[-1]}"
    echo "[GWF] $(date) completed DVFM latent-regularization v2"
    """
    inputs = [
