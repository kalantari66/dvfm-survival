"""One-job GWF workflow for the DVFM decoder calibration ablation.
...
CONFIG_PATH = PROJECT_ROOT / "configs" / "synthetic_dependence_calibration.yaml"


def run_calibration(config_path: Path, result_dir: Path, resources: dict) -> AnonymousTarget:
    outputs = [
    ...
    echo "[GWF] $(date) starting DVFM dependence-calibration ablation"
    ...
    {checks}
    touch "{outputs[-1]}"
    echo "[GWF] $(date) completed DVFM dependence-calibration ablation"
    """
    inputs = [
        config_path,
...
gwf.target_from_template(
    ...
    template=run_calibration(CONFIG_PATH, result_dir, config["resources"]),
)
